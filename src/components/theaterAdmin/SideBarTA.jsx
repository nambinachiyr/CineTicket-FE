...
const SideBarTA = ({data,setOpenSideBar}) => {
  ...
  return (
    <div className={`fixed text-[17px] rounded-tl-3xl  rounded-br-3xl md:text-2xl flex flex-col gap-5 md:gap-10 p-4 top-35 left-5 w-50 md:w-59 md:h-[35%] h-70 bg-white ${moviesOpen?'min-h-[80vh] overflow-y-auto':''}`}>
        <img onClick={()=>setOpenSideBar(false)} className='w-7 h-7 top-1 right-1 block md:hidden absolute hover:cursor-pointer bg-gray-600' src={close} alt="" />
        ...
        <p  className='flex flex-row-reverse justify-between'>
            ...
            <span className='hover:text-blue-500 hover:font-semibold hover:scale-105 transition-all hover:tracking-widest duration-200 hover:cursor-pointer' >Screens</span>
        </p>
        <p   className='flex flex-row-reverse justify-between'>
        ...
        {
            moviesOpen?runningMovies?.map(movie=>(
              <div className='border border-gray-400 p-2 '>
                <p className='border-b'>{movie.title} </p>
                <p>showCount - {movie.showCount}</p>
              </div>
            )):<p></p>
        }
