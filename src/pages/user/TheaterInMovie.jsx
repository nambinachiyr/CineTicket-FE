...
console.log(selectDate)
 ...
 useEffect(()=>{
  ...
  return (
    <div className='bg-[#fdfdf6] p-4 min-h-screen'>
     ...
     {
       isData? <p className='flex min-h-screen justify-center items-center text-xl text-gray-700'>No shows ...</p>
    ...
    <div className='p-3 tracking-wider lg:grid lg:grid-cols-2 flex flex-col gap-5  md:gap-7'>
      ...
      <div className='flex justify-center lg:w-full lg:h-35'>
        {
          showsByDates?.map((show,index)=>(
            <div key={show._id} className='border gap-3 bg-gray-200 flex flex-col justify-center border-gray-400 rounded-lg flex-wrap w-[90%] md:w-[85%] p-2 md:px-6'>
            <p  className='flex text-[16px] font-medium text-neutral-800  md:text-lg items-center gap-2 lg:text-lg tracking-wide'>{show.theater.name}, <span className='flex items-center'><img src={placeholder} alt="" className='w-3 h-3 md:w-5 md:h-5' />{(show.theater.address).split(',').at(-1)}</span></p>
            <div className=''>
              {/* Time Section */}
              {
                show.showStatus!=="Completed"?<Link to={`/${show._id}/${show.showDate}/${show.showTime}`} key={index} className='hover:cursor-pointer tracking-normal border border-x-4 text-sm md:border-x-8 justify-center bg-gray-100 transition-all duration-300 hover:translate rounded-3xl hover:scale-105 shadow-2xl border-green-600 w-19 md:w-28 text-center whitespace-nowrap px-1 py-1 flex flex-wrap gap-3'>{show.showTime}</Link>:""
              }
